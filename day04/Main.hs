...
newtype Shift= Shift{unShift:: (String, LocalTime, [Period])} deriving Show


pt1 :: String -> Int
pt1 input =
  let linesPerShift = partitionAt (endswith "begins shift") $ sort $ lines input
      shifts = map parseShift linesPerShift
      sortedPerGuard = sortBy (\ (Shift(id1, _, _)) (Shift(id2, _, _))-> compare id1 id2) shifts
      groupedPerGuard = groupBy (\ (Shift(id1, _, _)) (Shift(id2, _, _))-> id1 == id2) sortedPerGuard
      guardWithMaxSleep = maximumBy (\ss1 ss2 -> compare (totalSleepTime ss1) (totalSleepTime ss2) ) groupedPerGuard
      guardId =read $ drop 1 $  extract_1 $ unShift $ head guardWithMaxSleep
      mostFrequentMinute = fst $ maxMinuteFreq guardWithMaxSleep
  in guardId * mostFrequentMinute
...
pt2 input =
  let linesPerShift = partitionAt (endswith "begins shift") $ sort $ lines input
      shifts = map parseShift linesPerShift
      sleepingShifts= filter isSleeping shifts
      sortedPerGuard = sortBy (\ (Shift(id1, _, _)) (Shift(id2, _, _))-> compare id1 id2) sleepingShifts
      groupedPerGuard = groupBy (\ (Shift(id1, _, _)) (Shift(id2, _, _))-> id1 == id2) sortedPerGuard
      guardWithMaxMinuteFreq = maximumBy (\ss1 ss2 -> compare (snd $ maxMinuteFreq ss1) (snd $ maxMinuteFreq ss2) ) groupedPerGuard
      guardId =read $ drop 1 $  extract_1 $ unShift $ head guardWithMaxMinuteFreq
      mostFrequentMinute = fst $ maxMinuteFreq guardWithMaxMinuteFreq
  in guardId * mostFrequentMinute


isSleeping:: Shift -> Bool
isSleeping (Shift(_, _, []))= False
isSleeping _= True

maxMinuteFreq:: [Shift] -> (Int, Int)
maxMinuteFreq ss =
  let periods = concatMap (extract_3 . unShift) ss
      minutes = concatMap toMinutesOfHour periods
      minuteFreq = group $ sort minutes
   in case maximumBy (\f1 f2 -> compare (length f1) (length f2)) minuteFreq of
        [] -> (-1,0)
        xs -> (head xs, length xs)

totalSleepTime:: [Shift] -> Int
totalSleepTime ss = sum $ map toShiftDuration ss

toShiftDuration:: Shift -> Int
toShiftDuration (Shift(_, _, periods)) = sum $ map toDuration periods

toDuration:: Period -> Int
toDuration (tod1, tod2) = todMin tod2 - todMin tod1 - 1

toMinutesOfHour:: Period -> [Int]
toMinutesOfHour (tod1, tod2) = [todMin tod1 .. (todMin tod2 - 1)]

parseShift:: [String]->Shift
parseShift ls =
  let startTime = extractTime $ head ls
      id = words (head ls) !! 3
      periodLines = Utils.partition 2 $ tail ls
      periods = map (\[l1, l2] -> parsePeriod l1 l2) periodLines
   in Shift (id, startTime, periods)


parsePeriod:: String -> String -> Period
parsePeriod l1 l2 =
  let start = extractTime l1
      end = extractTime l2
   in (localTimeOfDay start, localTimeOfDay end)

extractTime:: String -> LocalTime
extractTime s =
  let timePart = take (length "[1518-11-01 00:00]") s
  in fst $ fromJust $ strptime "[%Y-%m-%d %H:%M]" timePart
